...
- **Products:** each marketplace has its own list of products (Canada starts with the four Electrolytes flavours from the Slack kickoff). Switch between them in the product bar. Per-product tasks have a separate status for each product; once-per-launch tasks (tagged "Launch-wide") share one status across every product. **All products** shows a card per product (progress, blocking tasks, blocked tasks, next gate) and a grid comparing every per-product task across products. Add, rename or remove products with **Add product** and **Edit**. Products live in `data/products.js`; changes made in the dashboard are saved like statuses.
- **Gates per product:** a product's view shows its own gates; the All products view only passes a gate when every product (and the launch-wide tasks) has passed it.
